function plot_committor_only(x_t, x_A, x_B, position, coordinate_label, time_label, dt, xx, Fx)
...
axes_handles = [axes_handles ax]; % store axis handle
hold on;
fill([xbins fliplr(xbins)], [(pAx-2*dpAx) fliplr(pAx+2*dpAx)], lightgray, 'LineWidth', 1.0e-6); % 2 sigma % FIX ME
fill([xbins fliplr(xbins)], [(pAx-dpAx) fliplr(pAx+dpAx)], darkgray, 'LineWidth', 1.0e-6); % 1 sigma
plot(xbins, pAx, 'k-', 'LineWidth', 2.0); % MLE
axis([x_A x_B 0 1]);
h = ylabel('p_A'); label_handles = [label_handles h];
h = xlabel(coordinate_label); label_handles = [label_handles h];
set(gca, 'XTick', [x_A x_B]);
...
hold on;
plot(xbins, pAx_pmf, 'r--', 'LineWidth', 2.0); % PMF-based
