...
A_t(find(x_t <= x_A)) = 1;
B_t(find(x_t >= x_B)) = 1;
% generate a list of all transitions into absorbed region
disp('Finding transitions into absorbed region...');
free_indices = find((x_t > x_A) & (x_t < x_B));
free_t = zeros(size(x_t), 'int8');
free_t(free_indices) = 1;
absorbed_indices = find((x_t <= x_A) | (x_t >= x_B));
absorbed_t = zeros(size(x_t), 'int8');
absorbed_t(absorbed_indices) = 1;
transition_indices = find(free_t(1:(T-1)) & absorbed_t(2:T)) + 1; % indices where system has transitioned into absorbing boundary
% mark all transitions
disp('Marking all transitions...')
start_index = free_indices(1);
while ((start_index >= free_indices(1)) & (start_index < free_indices(end)))
  %start_index

  % find first sample in absorbed region
  index = find(transition_indices > start_index, 1);
  first_absorbed_index = transition_indices(index);
  ...
  index = find(free_indices > first_absorbed_index, 1);
  start_index = free_indices(index);
end
