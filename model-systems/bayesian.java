...
public class bayesian {
   ...
   *
   * @param s_t   trajectory of states
   * @param tau   lag time for accumulating counts
   * @return the matrix of transition counts where Nij(i,j) is number of observed counts starting in i and ending in j
   */
  public static double [][] compute_transition_counts(int [] s_t, int tau) {
    // Determine timeseries length.
    int T = s_t.length;

    // Determine number of discrete states.
    int nstates = max(s_t);
    
    // Allocate storage for transition counts.
    double [][] Nij = new double[nstates][nstates];
  ...
  /**
   * Compute transition counts for continuous trajectory, ignoring transitions into or out of a given interval.
   *
   * @param s_t   trajectory of states
   * @param tau   lag time for accumulating counts
   * @return the matrix of transition counts where Nij(i,j) is number of observed counts starting in i and ending in j
   */
  public static double [][] compute_restricted_transition_counts(double [] x_t, double xmin, double dx, int nbins, int tau) {
    // Determine timeseries length.
    int T = x_t.length;

    // Compute maximum of allowed interval.
    double xmax = xmin + dx*nbins;

    // Allocate storage for transition counts.
    double [][] Nij = new double[nbins][nbins];
    ...
    for(int t0 = 0; t0 < T-tau; t0++) {
      int i = (int)((x_t[t0]-xmin) / dx);
      int j = (int)((x_t[t0+tau]-xmin) / dx);
      if ((0 <= i) && (i < nbins) && (0 <= j) && (j < nbins))
        Nij[i][j]++;
    }
